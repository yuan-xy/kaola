...
REST规范没有说明如何实现批量操作，也没有说明提交参数和返回值的格式。实践中，elasticsearch提供了批量操作的入口/\_bulk，统一处理所有的批量操作，可以在一次请求里完成索引／更新／删除等多个操作。本方案对批量操作的要求更严格，只支持单表数据的一种操作。对于批量新增接口，我们重用Restful的新增接口，只是提交的数据格式不一样。
